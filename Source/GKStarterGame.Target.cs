...
public class GKStarterGameTarget : TargetRules
{
    public GKStarterGameTarget(TargetInfo Target) : base(Target)
    {
        ...
        GitCommand.AddVersionInfo(this, "GKSTARTER");
    }

    private static bool bHasWarnedAboutShared = false;


    internal static void ApplySharedGKSGTargetSettings(TargetRules Target)
    {
        ...
        if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
        {
            ...
            GKStarterGameTarget.ConfigureGameFeaturePlugins(Target);
        }
        else
        {
            // !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
            ...
            else
            {
                ...
                if (!bHasWarnedAboutShared)
                {
                    ...
                    Logger.LogWarning("GKSGGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
                }
            }
        }
    }
...
//
// If project is a git repository resolve git versioning
//
class GitCommand
{
    public static void AddVersionInfo(TargetRules Target, string Namespace)
    {
        // Version Info
        // ------------
        // Automatically set by the CI
        string TagDefault = "v0.0.0";
        string HashDefault = "outoftree";
        string DateDefault = "1900-01-01 00:00:00 -0000";

        string ProjectDirectory = Path.GetDirectoryName(Target.ProjectFile.FullName);


        string Tag = GetLatestTag(ProjectDirectory, TagDefault);
        string CommitHash = GetLatestCommit(ProjectDirectory, HashDefault); 
        string CommitDate= GetCommitDate(ProjectDirectory, CommitHash, DateDefault);

        Target.ProjectDefinitions.Add(Namespace + "_TAG=" + Tag);
        Target.ProjectDefinitions.Add(Namespace + "_HASH=" + CommitHash);
        Target.ProjectDefinitions.Add(Namespace + "_DATE=" + CommitDate);
    }

    static string GetLatestTag(string Folder, string DefaultTag)
    {
        GitCommand GitCommitHashProcess = new GitCommand("git", Folder, "--no-optional-locks describe --tags --abbrev=0", DefaultTag);
        GitCommitHashProcess.WaitForBackgroundProcess();
        return GitCommitHashProcess.GetResult();
    }

    static string GetLatestCommit(string Folder, string DefaultCommit)
    {
        GitCommand GitCommitHashProcess = new GitCommand("git", Folder, "--no-optional-locks rev-parse HEAD", DefaultCommit);
        GitCommitHashProcess.WaitForBackgroundProcess();
        return GitCommitHashProcess.GetResult();
    }

    static string GetCommitDate(string Folder, string Commit, string DefaultDate)
    {
        GitCommand GitCommitHashProcess = new GitCommand("git", Folder, "--no-optional-locks show -s --format=%ci " + Commit, DefaultDate);
        GitCommitHashProcess.WaitForBackgroundProcess();
        return GitCommitHashProcess.GetResult();
    }
    ...
    public GitCommand(string GitPath, string RootDir, string Arguments, string Default)
    {
        ...
        BackgroundProcess.StartInfo.Arguments = Arguments;
        BackgroundProcess.StartInfo.WorkingDirectory = RootDir;
    ...
    public void WaitForBackgroundProcess()
    {
        if (BackgroundProcess != null)
        {
            if (!BackgroundProcess.WaitForExit(2000))
            {
                try
                {
                    BackgroundProcess.Kill();
